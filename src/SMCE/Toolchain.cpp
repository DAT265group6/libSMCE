...
struct toolchain_error_category : public std::error_category {
  public:
    ...
    std::string message(int ev) const override {
        ...
        case toolchain_error::cmake_outdated:
            return "CMake outdated";
...
[[nodiscard]] std::error_code Toolchain::check_suitable_environment() noexcept {
    ...
    if (m_cmake_path != "cmake") {
        ...
        if (m_cmake_path.empty())
            ...
            std::string min_cmake_version = "3.12.0";
            // Find CMake in RtResources
            // Find CMake in Environment Variables
            std::string cur_cmake_version = "";
            std::cout << "TestTest" << std::endl;
            std::cout << "TestTest" << std::endl;
            std::cout << "TestTest" << std::endl;
            // TODO: Check CMake outdated or not
            // Compare versions

            return toolchain_error::cmake_outdated;
        }
    }
